...
```{r}
estimate_tmle <- function(data, true_value = 0.212, alpha = 0.05, weight = TRUE, use_single_H = TRUE, transforms = TRUE) {
  ...
  if(all(data$Y %in% c(0, 1))){
    ...
    b <- max(data$Y)
    data$Y <- (data$Y - a) / (b - a)
    update_family <- quasibinomial
    q_family <- gaussian
  }else{
    update_family <- gaussian
    q_family <- gaussian
  }
  
  f_offset <- function(x){
    ifelse(outcome == 'binary', logit(x) , ifelse(transforms, logit(x), x))
  }
  
  f_star <- function(x){
    ifelse(outcome == 'binary', expit(x) , ifelse(transforms, expit(x), x))
  }
  ...
  f_offset <- function(x,condition){
    ifelse(condition, logit(x), x)
  }
  
  print(head(f_offset(data$Q, transforms)))
  
  if(outcome == 'binary'){
    data$offset <- logit(data$Q)
  }else if (transforms) {
    data$offset <- logit(data$Q)
  }else{
    data$offset <- data$Q
  ...
  formula_g <- as.formula(paste("T ~ W1 + W2 + W3"))
  g_init <- glm(formula_g, family = binomial, data = data)
  data$g1W <- predict(g_init, type = "response")
  ...
  if (use_single_H) {
    second_stage <- glm2(data$Y ~ -1 + H + offset(data$offset), 
                         family = update_family, data = data, weights = weights_single_H)
    epsilon <- coef(second_stage)[1]
    Q_star1 <- f_star(f_offset(data$Q1W, transforms) + epsilon * data$k_Q1)
    Q_star0 <- f_star(f_offset(data$Q0W, transforms) - epsilon * data$k_Q0)
  } else {
    second_stage_1 <- glm2(data$Y ~ -1 + H1 + offset(data$offset), 
                           family = update_family, data = data, weights = weights_H1)
    second_stage_0 <- glm2(data$Y ~ -1 + H0 + offset(data$offset), 
                           family = update_family, data = data, weights = weights_H0)
    epsilon_1 <- coef(second_stage_1)[1]
    epsilon_0 <- coef(second_stage_0)[1]
    Q_star1 <- f_star(f_offset(data$Q1W, transforms) + epsilon_1 * data$k_Q1)
    Q_star0 <- f_star(f_offset(data$Q0W, transforms) + epsilon_0 * data$k_Q0)
  }
